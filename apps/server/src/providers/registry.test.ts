...
describe("provider registry", () => {
  it("registers the providers (incl. v5.0 smart-life + B1-B7 integrations)", () => {
    ...
    // E6 — more keyless providers (geo / civic / earth-science / name-novelty / reference)
    for (const id of ["bigdatacloud", "postcodesio", "censusgeocoder", "policeuk", "fbiwanted", "ukbills", "agify", "genderize", "nationalize", "noaatides", "usgswater", "citypopulation"]) expect(PROVIDERS.has(id)).toBe(true);
    for (const id of ["bigdatacloud", "postcodesio", "censusgeocoder", "policeuk", "fbiwanted", "ukbills", "agify", "genderize", "nationalize", "noaatides", "usgswater", "citypopulation"]) expect(PROVIDERS.get(id)?.authKind).toBe("none");
