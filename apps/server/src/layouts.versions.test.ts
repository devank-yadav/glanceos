...
describe("board version history", () => {
  it("archives the replaced document on each (throttle-spaced) save and restores it", () => {
    const layout = createLayout("D0", blankDocument("D0"), { userId: user.id });
    ...
    const versions = listLayoutVersions(layout.id, user.id);
    expect(versions.length).toBe(2);
    expect(versions[0]!.summary).toBe("D1"); // newest first
    expect(versions[1]!.summary).toBe("D0");

    // restore the oldest (D0)
    const oldest = getLayoutVersionDocument(layout.id, versions[1]!.id, user.id);
  ...
  it("throttles rapid saves to one snapshot", () => {
    const layout = createLayout("R0", blankDocument("R0"), { userId: user.id });
    ...
    expect(listLayoutVersions(layout.id, user.id).length).toBe(1);
  });

  it("only the owner can see a board's versions", () => {
    const layout = createLayout("P0", blankDocument("P0"), { userId: user.id });
