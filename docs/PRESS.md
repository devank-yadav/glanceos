...
bind live data from 190 keyless and connected sources, and run it on your own
...
glanceable dashboards. You build boards in a Notion-style editor from 221 block
types — clocks, weather, lists, stats, gauges, countdowns, signage — and any
block can be made *live*, pulling from 190 server-side integrations (over half
need no login at all) or any public URL. Boards stream to connected screens over
SSE and render through a sub-32 KB runtime, so they work on old smart-TVs and
...
| **Blocks** | 221 block types |
| **Templates** | 166 ready-made full-screen boards (incl. zero-setup live ones) |
| **Integrations** | 190 server-side data sources (~108 keyless / no login) |
| **Preset objects** | 104 one-click pre-bound blocks |
| **Screens** | Any browser — TV, tablet, monitor, e-ink, Raspberry Pi; native shells for Android TV/Fire TV, Tizen, webOS, tvOS |
| **Runtime size** | Screen runtime ≤ 32 KB gzipped (CI-gated) |
