...
- [~] **A5 — README + landing accuracy + unfurl** — **done (server-injected absolute meta is the remaining nice-to-have).** ✅ Fixed stale counts in README (85→**121** data sources/providers, 211→**213** block types, "all 199"→**213**, 399→**510+** tests) and on the landing ("Connect 85 apps"→**121**). ✅ Added a **GitHub repo link** twice on the landing (hero "View on GitHub ↗" CTA + footer link) — verified live logged-out (2 anchors render, calm monochrome). ✅ Added **OG/Twitter card meta** to `apps/config/index.html` (og:type/site_name/title/description/image + twitter summary_large_image) pointing at a baked **`apps/config/public/og.png`** (1200×630, sharp-rendered brand card — wordmark + glyph + tagline + "213 blocks · 153 templates · 121 integrations · MIT"); dev server serves `og.png` 200 and the tags appear in the served `/`. **Deferred (nice-to-have):** server-injected absolute `og:url`/`og:image` for `/` keyed off `GLANCEOS_PUBLIC_URL` (relative `/og.png` already resolves against the page origin for most scrapers); trimming the long README changelog blob (left intact — accurate, just verbose).
